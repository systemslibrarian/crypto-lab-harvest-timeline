 ...
 *    files the node under `incomplete`. Every verdict, badge and stat on this
 *    page sits on a translucent `*-dim` tint (`rgba(255, 51, 102, 0.15)` and
 *    friends) layered over `--color-surface`, the hero's "why it matters" card
 *    is a `color-mix(in oklab, ...)` wash, and `.harvest-step-icon` paints
 *    `rgba(0, 0, 0, 0.18)` over whatever the step behind it is. None of those
 *    is a flat colour axe can read off one declaration.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.mosca-bar-z` renders at
 *    `opacity: 0.75` and `.seg-y` at `0.85`; the `.gloss-pop` jargon popover and
 *    the exposure-curve `.chart-tooltip` both live at `opacity: 0` until hover
 *    or focus brings them up, which is a state the gate builds and scans.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Every wide table
 *    here lives in a `.table-scroll` (`overflow-x: auto`), and at 380px the
 *    six-column organization profile, the cost-of-delay grid and the exhibit-6
 *    record list are mostly clipped away. Content scrolled past the client box
 *    is not dimmed or partly drawn — it is absent from the frame, and asking
 *    what colour it sits on has no answer. Its rect is still to the right of
 *    every ancestor's box, so the ancestor walk would find nothing behind it and
 *    fall through to white, inventing a failure per cell. Skip it, and rely on
 *    the wider viewport where the very same cell is visible and measured for
 *    real.
 *
 ...
 *    and reports a fixed 1:1. Nothing on this page does that today; the guard is
 *    two lines and silent when unused.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. Exhibit 3
 *    is a hand-built `<svg>` exposure chart: it draws its plot area as a `<rect>`
 *    and then its axis labels, tick values and curve annotations as following
 *    `<text>` elements. A label's backdrop is therefore a PRECEDING SIBLING
 *    shape, not an ancestor. An ancestor-only walk would composite those labels
 *    onto the svg's own transparent background and report a ratio nothing on
 *    screen has. So for SVG text, any earlier sibling shape whose box contains
 *    the text box is composited first — and SVG text takes its ink from `fill`,
 *    not `color`, which the main loop handles separately.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" card is `color-mix(in oklab, var(--color-today) 6%, transparent)`
     * and the shared top bar mixes in `srgb`, and Chromium reports both to
     * `getComputedStyle` unchanged rather than converting them to sRGB. A
     ...
     * fabricates failures (a verdict's dark ink read as sitting on the page
     * ground rather than on its own `*-dim` tint) and, far worse, could hide a
     * real one. The 2D canvas is the browser's own colour
     ...
     *
     * An earlier form of this helper judged every gradient at its worst *stop*,
     * assuming that stop covered the text wherever the text sat. That is right
     * for a gradient that spans its element and wrong for a small one, or for
     * one whose worst stop is nowhere near the text — and judging a page-tall
     * element by a corner stop invents backdrops nothing sits on. So each
     * gradient is *sampled* at the text's real location instead: linear by
     * projecting onto the gradient line, radial by distance from the centre over
     * the farthest-corner radius. A non-gradient layer (`url()`, `none`) is
     ...
     *
     * Six exhibits, four of them table-driven: the organization profile, the
     * cost-of-delay grid, the personal-application table and the exhibit-6
     * record list together put hundreds of text-owning cells on the page, and
     * every one of them walks the same handful of exhibit ancestors. Without the
     ...
     * surface it happens to sit on and INVENTS a failure per instance, for text
     * nobody can see. Deliberately narrow — only a ZERO-area clip qualifies, so
     * a real partial clip is still measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosure is the `.sources-panel` citation
      // grid; the gate opens it by clicking its summary and scans the open state
      // for real, rather than force-setting `.open` as the old spec did.
      //
      // The same test also covers `.gloss-pop`, which is `visibility: hidden`
      // until its `.gloss` term is hovered or focused — a state the gate builds.
      ...
      // `top: -3rem` and the lab's own `.skip-link` parks off-screen, each
     ...
     * it the three `.harvest-step-icon` emoji (📥 🗄️ 🔓) are measured as if each
     * glyph took the element's declared `color` — but an emoji paints in its own
     * multicolour, so that ratio describes nothing on screen. The `.sort-arrow`
     * glyphs and the `.mosca-bar` segment fills are `aria-hidden` for the same
     * reason: a sighted duplicate of information the row already states in text.
     *
     * NOTE THE COST, because it is a real blind spot: `aria-hidden` removes text
     * from the accessibility tree, not from the screen, and WCAG 1.4.3 is about
     * the screen. Every `aria-hidden` element that owns visible text on this
     * page was enumerated and measured by hand instead.
     */
     ...
     * is transparent the value comes from <body> instead. This page sets
     * `html { background-color: var(--color-bg) }` and, with six exhibits open,
     * runs a dozen viewports tall — so the canvas is load-bearing here: most of
     * the document is nowhere near the root's own box. A lab that sets
     * `html, body { height: 100% }` on a tall document has both boxes exactly
     * one viewport tall, every element below the fold intersects neither, the
     * walk ends transparent, and it falls through to WHITE — reporting
     * dark-theme text against a page that does not exist. In one lab that was 34
     * of 38 findings in a single run, and it can mask a real failure in the
